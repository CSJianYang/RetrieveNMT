...
from fairseq.modules import (
    ...
    LearnedPositionalEmbedding, MultiheadAttention, SinusoidalPositionalEmbedding, MultiheadAttentionRelativePosition
)
...
@register_model('transformer_masked_lm')
class transformer_masked_lm(FairseqModel):
    ...
    def add_args(parser):
        """Add model-specific arguments to the parser."""
        # fmt: off
        parser.add_argument('--pretrain', action='store_true', help='if pretrain')
        parser.add_argument('--pretrain-encoder', action='store_true', help='if pretrain encoder')
        parser.add_argument('--pretrain-decoder', action='store_true', help='if pretrain decoder')
        parser.add_argument('--dropout', type=float, metavar='D',
                            help='dropout probability')
        parser.add_argument('--attention-dropout', type=float, metavar='D',
                            help='dropout probability for attention weights')
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens):
        ...
        if self.pretrain:
            encoder_out = self.encoder(src_tokens, src_lengths)
            decoder_out = self.decoder(src_tokens, encoder_out=encoder_out, pretrain=True)
        else:
            encoder_out = self.encoder(src_tokens, src_lengths)
            decoder_out = self.decoder(prev_output_tokens, encoder_out=encoder_out, pretrain=False)
    ...
    def build_model(cls, args, task):
        ...
        encoder = TransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TransformerDecoder(args, tgt_dict, decoder_embed_tokens, no_encoder_attn=args.pretrain, final_norm=True)
        return transformer_masked_lm(encoder, decoder, args)
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        #     )
        # else:
        #     self.self_attn = MultiheadAttentionRelativePosition(
        #         self.embed_dim, args.encoder_attention_heads,
        #         dropout=args.attention_dropout, padding_idx=args.padding_idx,
        #         max_target_positions=args.max_source_positions
        #     )
...
@register_model_architecture('transformer', 'transformer_MASS')
def transformer_wmt_en_de_MASS(args):
    base_architecture(args)
    args.criterion = getattr(args, 'MASS_loss', 'MASS_loss')

@register_model_architecture('transformer_masked_lm', 'transformer_masked_lm_base')
def transformer_masked_lm_base(args):
    my_transformer(args)
    #base_architecture(args)
    args.pretrain = getattr(args, 'pretrain', False)
    args.pretrain_encoder = getattr(args, 'pretrain_encoder', False)
    args.pretrain_decoder = getattr(args, 'pretrain_decoder', False)
    if args.pretrain:
        args.criterion = getattr(args, 'bert_loss', 'bert_loss')
