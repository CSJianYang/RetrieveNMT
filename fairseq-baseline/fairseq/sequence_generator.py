    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if "target_tokens" in kwargs.keys() and step > 0 and step < kwargs["target_lengths"].cpu().numpy()[0]:
                tokens.index_fill_(1, torch.tensor([step]).cuda(), kwargs["target_tokens"][0][step - 1].cuda())
                #tokens[step] = kwargs["target_tokens"][0][step].repeat(beam_size)
