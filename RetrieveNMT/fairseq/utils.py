...
def load_model_state(filename, model, reset_target_embedding=False):
    if not os.path.exists(filename):
        return None, [], None
    state = torch.load(filename, map_location=lambda s, l: default_restore_location(s, 'cpu'))
    state = _upgrade_state_dict(state)
    if reset_target_embedding:
        print("reset target embedding !")
        state['model'].pop('decoder.embed_tokens.weight')
        state['model'].pop('decoder.embed_positions._float_tensor')
