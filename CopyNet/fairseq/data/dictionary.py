...
class Dictionary(object):
    ...
    def finalize(self, threshold=-1, nwords=-1, padding_factor=8):
        ...
        for symbol, count in c.most_common(nwords - self.nspecial):
            if count >= threshold:
                ...
                break
            if len(new_symbols) != len(new_indices):
    ...
    def dummy_sentence(self, length):
        t = torch.Tensor(length).uniform_(self.unk_index, self.unk_index+1).long()
        t[-1] = self.eos()
        return t

    def encode_maskline(self, line, line_tokenizer=tokenize_line, add_if_not_exist=True,
