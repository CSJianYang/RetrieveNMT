    ...
    def Source_RetrieveSource_RetrieveTarget_Split(x, l, args=None):
        ...
        for i in range(l.size(0)):
            ...
            if append_position > 0 and append_position < words.size(0):
                ...
                retrieve_words = x.new([args.APPEND_ID] + args.retrieve_number * [args.SRC_ID,args.unk_idx,args.TGT_ID,args.unk_idx,args.SEP_ID])
            else:
                source_words = words
                retrieve_words = x.new([args.APPEND_ID] + args.retrieve_number * [args.SRC_ID,args.unk_idx,args.TGT_ID,args.unk_idx,args.SEP_ID])
            ...
            #if len(sep_positions) != args.retrieve_number or len(src_positions) != args.retrieve_number or len(tgt_positions) != args.retrieve_number:

            assert len(sep_positions) == args.retrieve_number and len(sep_positions) == len(src_positions) and len(sep_positions) == len(tgt_positions), "Please make sure [SEP] [SRC] [TGT] have the same number of occurrences"
        ...
        for i in range(min(args.retrieve_number,len(sep_positions))):
            ...
            assert x.size(0) == retrieve_source_x.size(0), "x: {}, retrieve_source_x: {}".format(x.size(),retrieve_source_x.size())
            assert x.size(0) == retrieve_target_x.size(0), "x: {}, retrieve_target_x: {}".format(x.size(),retrieve_target_x.size())
